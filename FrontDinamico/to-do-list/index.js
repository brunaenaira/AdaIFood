...
function addTask() {
    ...
    const concluded = document.getElementById("concluded").checked;
    ...
    const newTask = {
        id, title, description, concluded
    };
    tasks.push(newTask);
    saveLocalStorage();
...
function editTask() {
    ...
    const newConcluded = document.getElementById("concluded").checked;
    ...
    selectedTask.concluded = newConcluded;
    tasks = tasks.map(task => task.id === selectedTaskId ? selectedTask : task);
    saveLocalStorage();
...
function addTaskForm() {
    document.getElementById("output").innerHTML = `
                ...
                <form onsubmit="event.preventDefault(); addTask()" id="form-add">
                    ...
                    <label for="concluded">Está concluída?</label>
    ...
    tasks.forEach(task => {
        taskList += `
                    <tr>
                        ...
                        <td>${task.concluded ? 'Sim' : 'Não'}</td>
<!--                        <td><input type="checkbox" id="concluded" checked="${task.concluded}" onchange="handleChange(this);"></td>-->
                    </tr>
                `;
    });
...
function handleChange(element) {
    element.parent.classList.toggle('completed');
    saveTasks();
}